...
| 機能 | CQOx | [Optimizely](https://www.optimizely.com/) | [VWO](https://vwo.com/) | [AB Tasty](https://www.abtasty.com/) | [Dynamic Yield](https://www.dynamicyield.com/) |
|------------|------|------------|-----|----------|---------------|
| **因果推論手法** | 7つの推定器（DR, IPW, DiD, IV, CF, SCM, RD） | A/B テストのみ | A/B テストのみ | A/B テストのみ | A/B テストのみ |
| **選択バイアスの除去** | Doubly Robust + Propensity Score | ❌ 完全なランダム化が必要 | ❌ 完全なランダム化が必要 | ❌ 完全なランダム化が必要 | ❌ 完全なランダム化が必要 |
| **異質な処置効果（CATE）** | ✅ Causal Forest による顧客レベル効果 | ❌ 平均効果のみ | ❌ 平均効果のみ | △ 事前定義セグメント | △ 事前定義セグメント |
| **反事実シミュレーション** | ✅ デプロイ前に ROI を予測 | ❌ 実験を実行する必要あり | ❌ 実験を実行する必要あり | ❌ 実験を実行する必要あり | △ 限定的なシナリオ |
| **長期効果予測** | ✅ DiD + 時系列（6ヶ月予測） | ❌ 短期のみ | ❌ 短期のみ | ❌ 短期のみ | ❌ 短期のみ |
| **操作変数法** | ✅ 内生性/交絡因子を処理 | ❌ 非対応 | ❌ 非対応 | ❌ 非対応 | ❌ 非対応 |
| **施策最適化** | ✅ Pareto Frontier（利益-リスク-信頼度） | △ 基本ルール | ❌ 最適化なし | △ 基本ルール | △ 基本ルール |
| **SQL ベースセグメンテーション** | ✅ 任意の WHERE 句 | ❌ UI に制限 | ❌ UI に制限 | △ 限定 | △ 限定 |
| **デプロイメント** | ✅ オープンソース、自己ホスト、K8s 対応 | SaaS のみ | SaaS のみ | SaaS のみ | SaaS のみ |
| **価格** | **価格についてはお問い合わせください** | 年 $200k 以上 | 年 $100k 以上 | 年 $150k 以上 | 年 $250k 以上 |

**その他の商用ソリューション:**
- **[Optimize Next](https://optimize-next.com/)** - Google Optimize の代替として、同様の A/B テスト機能を提供
- **[SiTest](https://sitest.jp/)** - Web 最適化の日本市場リーダー（日本）
- **[DLPO](https://dlpo.jp/)** - ランディングページ最適化プラットフォーム（日本）
- **[Juicer](https://juicer.cc/)** - ユーザー行動分析とパーソナライゼーション（日本）

**なぜ CQOx か？** CQOx は因果推論により完全なランダム化の必要性を排除し、商用ツールでは扱えない観測データ（過去のキャンペーン、自然実験）から ROI を測定できるようにします。
...
CQOx は、専門的な因果推論およびマーケティングミックスモデリング（MMM）プラットフォームと同じ「**インクリメンタリティ測定**」領域に属しています。しかし、ポジショニングと価値提案は大きく異なります：

**主要なインクリメンタリティ & 因果測定プレイヤー:**
- **[Measured](https://www.measured.com/)** - マーケティングインクリメンタリティ測定 SaaS
- **[Lifesight](https://lifesight.io/)** - マーケティングアトリビューションおよびインクリメンタリティプラットフォーム
- **[Liftlab](https://www.liftlab.io/)** - 成長チーム向けインクリメンタリティテスト
- **Haus / Incrmnta / Sellforte** - MMM およびインクリメンタリティ SaaS プロバイダー

| 次元 | CQOx | Measured / Lifesight / Liftlab | Haus / Incrmnta / Sellforte | Uplift コンサルティングファーム |
|-----------|------|--------------------------------|------------------------------|-------------------------|
| **プロダクト vs コンサル依存度** | **セルフサービスプロダクト**。CSV/Parquet をアップロードすれば、アナリストはベンダーサポートなしで独立して分析を実行可能 | 主にセルフサービス（オンボーディングサポート付き） | ツール + ベンダーサポートが必要。初期セットアップと設計には通常外部リソースが必要 | ほぼ完全にコンサルティング主導。分析からインサイト提供まで外部チームに依存 |
| **因果推論の透明性** | **20以上の推定器（DR/IPW/DiD/IV/CF/SCM/RD）をOSSとして実装**。アルゴリズムは社内で検証・拡張可能 | 一部の方法論は開示されているが、独自実装 | 一部の実装はブラックボックス。モデリング詳細と再現可能なコードは提供されないことが多い | 分析ロジックはレポートに要約されるのみ。コードとモデルは通常提供されない |
| **自己ホスティング / セキュリティ要件** | **自己ホスト可能（オンプレ / VPC / K8s）**。データはインフラから出ない | 管理型 SaaS のみ。データはベンダーのクラウドにアップロードする必要がある | 主に管理型 SaaS。厳格なPIIや規制要件での使用は困難 | 分析にはデータ転送が必要。NDA下で運営されるが、定期的なデータエクスポートを前提 |
| **マルチ推定器 & 品質ゲート** | **主要推定器7つ + OPE/g-computation の組み合わせ**。品質ゲート（Overlap、weak IV、RD manipulation テスト）を UI で強制 | プラットフォーム固有の方法論（単一アプローチが多い） | 特定手法に焦点を当てた評価。品質検査の内部はツール依存で不透明なことが多い | プロジェクトごとのアドホックな手法選択。品質基準は案件によって異なる |
...
**各推定器は、異なるデータ構造と因果推論の課題に対して最適化されています。**
...
**使用例:**
- **マーケティングキャンペーン**：処置の割り当てが非ランダムな場合（例：高価値顧客がメールをより頻繁に受信）
- **製品ローンチ**：自己選択バイアスがある場合（早期採用者が一般人口と異なる）
- **医療介入**：処置の割り当てが患者の特性に依存する場合
- **価格実験**：割引が特定の顧客セグメントにターゲットされている場合
- モデルの誤特定がある場合でも、堅牢な因果推定値が必要なあらゆるシナリオ

**解決する問題:**
**選択バイアス & モデルの堅牢性** - 観測データでは、処置群と対照群が系統的に異なります（交絡）。従来の手法では、以下のいずれかが必要です：
- 完璧な成果モデル（回帰）または
- 完璧な傾向スコアモデル（重み付け）

DR-Learner は**二重の保護**を提供します：どちらかのモデルが正しく指定されていれば、推定値は一貫性を保ちます。これにより、純粋な回帰アプローチや純粋な IPW アプローチよりも、モデルの誤特定に対してより堅牢になります。
...
where:
  μ̂₁(X), μ̂₀(X) = 処置群/対照群の成果モデル（E[Y|X,T=1], E[Y|X,T=0]）
  ê(X) = 傾向スコア P(T=1|X)（共変量が与えられた場合の処置の確率）
  Tᵢ ∈ {0,1} = ユニット i の処置インジケーター
  Yᵢ = ユニット i の観測成果
  Xᵢ = ユニット i の共変量ベクトル

主要特性: 二重堅牢性 - μ̂ または ê のどちらかが正しく指定されていれば一貫性を持つ
```

**実例:**
- **シナリオ**: メールキャンペーンの ROI 測定
- **課題**: 高価値顧客がメールを受信する可能性が高い（選択バイアス）
- **解決策**: DR-Learner がサンプルを再重み付けしてバイアスを除去
- **結果**: 真の効果 = +¥240万（単純な比較 = +¥380万、58%の過大評価）
...
**使用例:**
- **ターゲット介入**：処置確率がグループ間で大きく異なる場合（例：非アクティブユーザーのみが割引を受ける）
- **調査データ**：非ランダムサンプリング（母集団を表現するために再重み付けが必要）
- **観測研究**：処置割り当てメカニズムがよく理解されているが複雑な場合
- **政策評価**：適格性ルールが処置確率の自然な変動を生み出す場合
- 観測データから**ランダム化をシミュレート**したい状況

**解決する問題:**
**非ランダムな処置割り当て & 交絡** - 多くの実世界のシナリオでは、処置割り当ては観測可能な特性（X）によって決定されます。例えば：
- マーケティングチームが高価値顧客をターゲット → 処置確率 ê(X) は X = "高 CLV" の場合に高い
- 非アクティブユーザーが再獲得オファーを受ける → ê(X) は X = "最近の活動が少ない" の場合に高い

IPW は、観測値を処置確率の逆数で再重み付けすることにより、**擬似ランダム化試験を再構築**します：
- **重みを増やす**：まれな観測値（例：非アクティブユーザーがまれに処置を受ける場合、処置を受けたときに重みを増やす）
- **重みを減らす**：過剰に表現されている観測値（例：アクティブユーザーが常に処置を受ける場合、重みを減らす）

これにより、処置群と対照群が**交換可能**になり、ランダム割り当てを模倣します。
...
where:
  Tᵢ ∈ {0,1} = 処置インジケーター
  Yᵢ = 観測成果
  ê(Xᵢ) = P(Tᵢ=1|Xᵢ) = 推定傾向スコア

解釈:
  - 処置ユニットは 1/ê(X) で重み付け → X が与えられた場合に処置がまれであれば重みを増やす
  - 対照ユニットは 1/(1-ê(X)) で重み付け → X が与えられた場合に対照がまれであれば重みを増やす

直感: 高価値顧客（ê=0.9）が処置を受けた場合、重み = 1/0.9 ≈ 1.1（小さな調整）
      低価値顧客（ê=0.1）が処置を受けた場合、重み = 1/0.1 = 10（大きな調整）
```

**実例:**
- **シナリオ**: ターゲット割引キャンペーン（非アクティブユーザーのみに送信）
- **課題**: ランダム化なし - すべての処置ユニットが非アクティブ
- **解決策**: IPW がランダム化試験を模倣するように再重み付け
- **結果**: 間違ったセグメントへの展開による -¥520万の損失を回避
...
**使用例:**
- **地理的変動がある政策介入**：ある州で新しい規制が制定され、他の州では制定されない場合
- **マーケティングキャンペーン**：テストとしてある地域/期間にローンチ
- **TV/ラジオ広告**：地域別メディア購入（一部の市場は露出、他の市場は非露出）
- **製品ローンチ**：店舗や都市間で段階的に展開
- **マクロ経済イベント**：処置群と対照群の両方に影響を与える場合（トレンドを差し引く必要がある）
- **前後データと比較群**があるあらゆるシナリオ

**解決する問題:**
**時不変交絡 & 長期トレンド** - 単純に前後の成果を比較することは誤解を招きます：
- **交絡トレンド**: 経済成長、季節性、市場成熟化が処置群と対照群の両方に影響
- **時不変の違い**: 処置地域は対照地域と本質的に異なる可能性がある

DiD は**二重差分**によってバイアスを除去します：
1. **第一差分（群内）**: 時系列での処置変化 - 時不変特性を除去
2. **第二差分（群間）**: 処置変化と対照変化を比較 - 共通の時間トレンドを除去

**仮定**: **平行トレンド** - 介入がなければ、処置群と対照群は同様に進化していたであろう。
...
where:
  Ȳₜᵀʳᵉᵃᵗ = 時点 t における処置群の平均成果（介入後）
  Ȳₜ₋₁ᵀʳᵉᵃᵗ = 時点 t-1 における処置群の平均成果（介入前）
  Ȳₜᶜᵒⁿᵗʳᵒˡ = 時点 t における対照群の平均成果
  Ȳₜ₋₁ᶜᵒⁿᵗʳᵒˡ = 時点 t-1 における対照群の平均成果

回帰仕様:
  Yᵢₜ = α + β₁·Treatᵢ + β₂·Postₜ + τ_DiD·(Treatᵢ × Postₜ) + εᵢₜ

  where τ_DiD は交互作用係数 = 因果効果

平行トレンド仮定:
  E[Yᵢₜ⁽⁰⁾ - Yᵢₜ₋₁⁽⁰⁾ | Treatᵢ=1] = E[Yᵢₜ⁽⁰⁾ - Yᵢₜ₋₁⁽⁰⁾ | Treatᵢ=0]
  （処置がなければ反事実トレンドは平行である）
```

**実例:**
- **シナリオ**: TV コマーシャルが売上に与える影響
- **課題**: 経済成長が両地域に影響
- **解決策**: DiD がコマーシャルの効果をマクロトレンドから分離
- **結果**: コマーシャルが売上を +¥890万押し上げ（単純な前後比較が示した +¥1200万ではない）
...
**使用例:**
- **逆因果関係**: アプリ使用が購入を増やすのか、それとも購入がアプリ使用を増やすのか？
- **省略変数バイアス**: 観測されない交絡因子（例：動機、健康状態）が処置と成果の両方に影響
- 処置変数の**測定誤差**
- **同時性**: 処置と成果が同時に決定される（例：需要供給における価格と数量）
- **自然実験**を見つけることができるシナリオ（設計ではなく、自然や政策におけるランダム性）

**解決する問題:**
**内生性 & 観測されない交絡** - 標準的な手法は以下の場合に失敗します：
1. **逆因果関係**: T → Y かつ Y → T（双方向因果）
   - 例: アプリ使用 → 購入、しかし購入 → アプリ使用でもある
2. **省略変数**: 観測されない U が T と Y の両方に影響
   - 例: 「動機」がジムへの参加（T）と体重減少（Y）の両方に影響
3. **測定誤差**: 真の処置 T* が T として誤測定される

IV は、以下のような**操作変数 Z**（外生的ショック）を見つけます：
- **処置に影響する**（Z → T）が、
- **処置を通じてのみ成果に影響する**（Z → T → Y、Z → Y の直接影響ではない）
- **交絡因子と無相関**（Z ⊥ U）

これにより、処置の**外生的変動**を分離して因果効果を推定します。
...
有効な操作変数 Z の要件:
1. 関連性: Cov(T, Z) ≠ 0  （Z が処置に影響 - 「第一段階」）
   - 検定: T ~ Z の回帰において F統計量 > 10
   - 弱い操作変数 → バイアス付き推定値

2. 除外制約: Z は T を通じてのみ Y に影響（直接的ではない）
   - Z → T → Y  （許可）
   - Z → Y      （違反 - Z が Y に直接影響）
   - 統計的に検定できない（維持仮定）

3. 外生性: Z ⊥ U （操作変数は観測不可能変数と無相関）
   - Z は交絡因子に対して「as-if ランダム」でなければならない
   - 制度的特徴や自然実験によって正当化されることが多い

二段階最小二乗（2SLS）実装:
  Stage 1: T̂ᵢ = α + γ·Zᵢ + εᵢ   （操作変数から処置を予測）
  Stage 2: Yᵢ = β + τ_IV·T̂ᵢ + ηᵢ  （予測処置に対して成果を回帰）

局所平均処置効果（LATE）:
  τ_IV は「コンプライアー」の効果を推定 - Z により処置状態が変化するユニット
```

**実例:**
- **シナリオ**: アプリ使用が購入に与える効果を測定
- **課題**: 逆因果関係（購入 → より多くのアプリ使用）
- **操作変数**: ランダムなプッシュ通知割り当て
- **解決策**: IV が使用が購入に与える効果を分離
- **結果**: アプリセッション 1 回追加 → +¥1,200 の収益（OLS が示唆する +¥3,400 ではない）
...
**使用例:**
- **パーソナライズドマーケティング**: どの顧客セグメントが割引、メール、広告に最もよく反応するか？
- **処置効果の異質性**: 同じ介入が年齢、地理、行動によって異なる効果を持つか？
- **ターゲティング最適化**: 期待リフトが最も高い顧客に限られた予算を割り当てる
- **精密医療**: どの患者が薬から最も恩恵を受けるか（副作用を経験するのは誰か）？
- **政策のパーソナライゼーション**: ワンサイズフィットオールではなく、個人の特性に合わせて介入を調整

**解決する問題:**
**異質な処置効果（HTE）& パーソナライゼーション** - 従来の手法は**平均処置効果**（ATE）を推定します：
```
τ_ATE = E[Y(1) - Y(0)]  （すべてのユニットにわたる平均効果）
```

しかし、効果は個人によって異なります：
- **割引**は価格に敏感な顧客の購入を増やすが、ロイヤル顧客からの収益を奪う
- **メールキャンペーン**は非アクティブユーザーのエンゲージメントを高めるが、アクティブユーザーを煩わせる
- **広告**は新規顧客を転換させるが、既存顧客にはゼロ効果

Causal Forest は**条件付き平均処置効果（CATE）**を推定します：
```
τ(x) = E[Y(1) - Y(0) | X = x]  （共変量 X が条件付けられた効果）
```

これにより以下が可能になります：
1. **ターゲティング**: τ(x) > コスト/閾値のユニットのみを処置
2. **層別化**: セグメント別に効果を報告（高/中/低反応者）
3. **最適割り当て**: 予算制約の下で ΣᵢτᵢDᵢ を最大化
...
where:
  Yᵢ(1) = ユニット i の処置下での潜在成果
  Yᵢ(0) = ユニット i の対照下での潜在成果
  X = 共変量ベクトル（年齢、CLV、購買履歴など）

Causal Forest アルゴリズム（Athey & Wager, 2016）:
  1. X で分割して処置効果の異質性を最大化するようにツリーを構築
  2. 正直分割: 分割と推定に異なるサンプルを使用（過剰適合を回避）
  3. 予測: X=x の新しいユニットについて、すべてのツリーからの予測を平均

分割基準（処置効果の分散を最大化）:
  標準回帰ツリーのように Σ(Yᵢ - Ŷᵢ)² を最小化する代わりに、
  葉間で Var[τ̂(x)] を最大化（効果の異質性を見つける）

正直推定:
  - データを I_split（ツリー構造用）と I_estimate（葉の予測用）に分割
  - 過剰適合を防ぎ、有効な信頼区間を提供

信頼区間:
  CQOx はブートストラップまたは無限小ジャックナイフにより τ̂(x) の CI を提供
```

**実例:**
- **シナリオ**: パーソナライズド割引ターゲティング
- **平均効果**: 顧客あたり +¥500
- **CATE 推定**:
  - 高価値顧客: 顧客あたり +¥2,300
  - 中価値顧客: 顧客あたり +¥450
  - 低価値顧客: **顧客あたり -¥180**（割引が定価販売を奪う）
- **アクション**: 高/中価値のみをターゲット → ROI が 1.4倍から 3.2倍に改善
...
**使用例:**
- **地理的介入**: 1つの都市/州/国での政策変更（例：カリフォルニアのタバコ税）
- **店舗レベルの展開**: 単一の場所でテストされる新しい店舗フォーマット
- **企業の合併・買収**: M&A が株価や収益に与える影響
- **規制変更**: GDPR がヨーロッパ企業 vs 米国企業に与える影響
- **自然災害**: 地震、ハリケーン、パンデミックが地域に与える経済的影響
- **1つの処置ユニット**と**複数の潜在的な対照**があるシナリオ

**解決する問題:**
**自然な対照群なし & N=1 問題** - 多くの政策/ビジネス決定には**単一の処置ユニット**が含まれます：
- カリフォルニアが炭素税を制定 → 「対照カリフォルニア」は存在しない
- 東京に新しい店舗がオープン → 東京はユニーク（同一の都市を見つけられない）
- ブレグジットが英国に影響 → 他の国は同時に EU を離脱していない

従来の手法は以下の理由で失敗します：
1. **完全一致なし**: 単一の対照ユニットが処置ユニットに完全に一致しない
2. **N=1**: 複数の処置ユニットを必要とする統計手法を使用できない
3. **集約データ**: 個人レベルのランダム化が不可能

SCM は、処置ユニットの**介入前の特性とトレンド**に最もよく一致する対照ユニットの加重平均を作成することにより、**合成対照**を構築します：
```
合成東京 = 0.4·大阪 + 0.3·名古屋 + 0.2·福岡 + 0.1·札幌
```

この合成ユニットは反事実として機能します：「介入がなければ東京はどうなっていたか？」
...
where:
  Ŷ₀ᵗ = 合成対照（時点 t における処置ユニットの反事実成果）
  Yⱼᵗ = 時点 t における対照ユニット j の観測成果
  wⱼ = 対照ユニット j に割り当てられた重み
  J = 潜在的な対照ユニットの数
...
最適化問題（介入前の不一致を最小化する重みを見つける）:
  w* = argmin Σₜ₌₁ᵀ⁰ (Y₁ᵗ - Σⱼ wⱼ·Yⱼᵗ)²

  where T₀ = 介入前期間、Y₁ᵗ = 処置ユニット成果

時点 t における処置効果:
  τ̂ₜˢᶜᴹ = Y₁ᵗ - Ŷ₀ᵗ = Y₁ᵗ - Σⱼ wⱼ·Yⱼᵗ   （t > T₀ の場合）

拡張 SCM（共変量 X を一致）:
  ||X₁ - Σⱼ wⱼXⱼ||² + λ·Σₜ₌₁ᵀ⁰ (Y₁ᵗ - Σⱼ wⱼYⱼᵗ)² を最小化

プラセボ検定による推論:
  各対照ユニットで SCM を実行 → 帰無仮説下での「効果」の分布
  P値 = 実際の効果の順位 / (J+1)
```
...
- **シナリオ**: 東京での新しい店舗オープン
- **課題**: 店舗の場所をランダム化できない
- **解決策**: SCM が大阪、名古屋、福岡の加重組み合わせから「合成東京」を作成
- **結果**: 店舗オープンが収益を +¥4500万押し上げ（都市レベルの成長から分離）
...
**使用例:**
- **閾値ベースのプログラム**: ¥50万円支出で VIP 会員、テストスコア > 70 で奨学金
- **年齢カットオフ**: 飲酒年齢、退職年齢、義務教育
- **地理的境界**: 選挙区、学校区、税務管轄区
- **時間カットオフ**: 政策の発効日（前後比較）
- **信用スコア**: 信用スコア閾値以上のローン承認
- **準ランダム化を生み出す明確な適格性カットオフ**があるあらゆる政策

**解決する問題:**
**閾値付近での観測可能変数による選択** - 多くの政策には観測可能な閾値に基づく適格性ルールがあります：
- 「年間支出 ≥ ¥50万円なら VIP 特典」
- 「入試スコア ≥ 70 なら奨学金」
- 「注文合計 ≥ ¥5,000 なら送料無料」

**問題**: 閾値をわずかに上回るユニットは、はるかに下回るユニットと系統的に異なります（交絡）
- 高支出者（¥100万円）≠ 低支出者（¥10万円）は多くの観測不可能変数（収入、ロイヤルティ、好み）で異なる

**解決策**: 閾値を**わずかに上回る vs わずかに下回る**ユニットを比較：
- ¥50.5万円支出の顧客 vs ¥49.5万円支出の顧客はほぼ同一
- 成果の違いは閾値をわずかに超えることにより**準ランダムに**割り当てられる
- これによりカットオフ付近で**局所ランダム化**が生まれる

RD は**閾値における因果効果**（限界的コンプライアーに対する LATE）を推定します。
...
where:
  c = 閾値（カットオフ）
  X = 連続変数（割り当て変数、例：支出、テストスコア）
  Y = 成果（例：リテンション、卒業率）
  μ⁺(c) = 上方から x→c として lim[E(Y|X=x)]（処置）
  μ⁻(c) = 下方から x→c として lim[E(Y|X=x)]（対照）

Sharp RD（処置がカットオフで 0 から 1 にジャンプ）:
  Tᵢ = 1[Xᵢ ≥ c]  （決定論的割り当て）

Fuzzy RD（処置確率がカットオフでジャンプするが、0 から 1 ではない）:
  P(Tᵢ=1|Xᵢ=c⁺) > P(Tᵢ=1|Xᵢ=c⁻)  （部分的なコンプライアンス）
  τ̂_Fuzzy = [E(Y|X=c⁺) - E(Y|X=c⁻)] / [E(T|X=c⁺) - E(T|X=c⁻)]

局所線形回帰（バンド幅 h）:
  [c, c+h] のデータを使用して μ⁺(c) を推定
  [c-h, c] のデータを使用して μ⁻(c) を推定

  最適バンド幅（MSE を最小化）:
    h* ∝ n^(-1/5)  （サンプルが大きい → バンド幅が狭い）

妥当性チェック:
  1. 連続変数の操作なし（McCrary 密度検定）
  2. カットオフにおける共変量の連続性（プラセボ検定）
  3. 偽のカットオフでの成果の不連続性なし
  4. バンド幅選択への感度

仮定:
  - 連続性: E[Y(0)|X=x] と E[Y(1)|X=x] は x=c で連続
  - 精密な操作なし: ユニットは閾値を超えるために X を完璧に制御できない
```

**実例:**
- **シナリオ**: VIP 会員特典（閾値：年間支出 ¥50万円）
- **課題**: 高支出者は低支出者と系統的に異なる
- **解決策**: ¥50万円をわずかに上回る/下回る顧客を比較（準ランダム）
- **結果**: VIP 特典がリテンションを 12 パーセントポイント増加
