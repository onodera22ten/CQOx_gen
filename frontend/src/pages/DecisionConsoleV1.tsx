...
const DecisionConsoleV1: React.FC = () => {
  ...
  // 円グラフ用データ
  const pieData = summary?.verdict_distribution ? [
    { name: 'Go', value: summary.verdict_distribution.go, color: '#10b981' },
    { name: 'Canary', value: summary.verdict_distribution.canary, color: '#f59e0b' },
    { name: 'Hold', value: summary.verdict_distribution.hold, color: '#ef4444' }
  ] : [];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <DeltaYenMetricCard
          title="今週のベストシナリオ Δ¥"
          value={summary ? formatYen(summary.max_delta_yen) : '-'}
        ...
        <DeltaYenMetricCard
          title="リスク高シナリオ数"
          value={summary?.verdict_distribution?.hold || 0}
          subtitle="Hold判定"
          icon="🔴"
          color="red"
        ...
        <DeltaYenMetricCard
          title="実行推奨キャンペーン"
          value={summary?.verdict_distribution?.go || 0}
          subtitle={summary ? `期待収益 ${formatYen(summary.max_delta_yen * (summary.verdict_distribution?.go || 0))}` : ''}
          icon="🟢"
          color="green"
        ...
        <DeltaYenMetricCard
          title="A/Bテスト候補"
          value={summary?.verdict_distribution?.canary || 0}
          subtitle="Canary判定"
          icon="🟡"
          color="yellow"
